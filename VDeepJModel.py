...
class VDeepJAllign(Model):
    def __init__(self, max_seq_length, vdj_class_holder):
        super(VDeepJAllign, self).__init__()
        self.max_seq_length = int(max_seq_length)
        self.v_family_count, self.v_gene_count, self.v_allele_count = (
            vdj_class_holder.v_family_count,
            vdj_class_holder.v_gene_count,
            vdj_class_holder.v_allele_count,
        )
        self.d_family_count, self.d_gene_count, self.d_allele_count = (
            vdj_class_holder.d_family_count,
            vdj_class_holder.d_gene_count,
            vdj_class_holder.d_allele_count,
        )
        self.j_gene_count, self.j_allele_count = (
            vdj_class_holder.j_gene_count,
            vdj_class_holder.j_allele_count,
        )
        ...
        self.total_ce_loss_tracker = tf.keras.metrics.Mean(name="total_ce_loss")
        ...
        self.v_mask_embedding_layer = tf.keras.layers.Embedding(
            5 + 1, 32, input_length=460
        )  # (masked_v)
        self.d_mask_embedding_layer = tf.keras.layers.Embedding(
            5 + 1, 32, input_length=460
        )  # (masked_d)
        self.j_mask_embedding_layer = tf.keras.layers.Embedding(
            5 + 1, 32, input_length=460
        )  # (masked_j)
        ...
        # TODO: @Thomas: Can this be a sigle layer?
        self.v_mask_extractor2 = ExtractGeneMask()  # (([tokenized_input],v_call_mask))
        self.d_mask_extractor2 = ExtractGeneMask()  # (([tokenized_input],d_call_mask))
        self.j_mask_extractor2 = ExtractGeneMask()  # (([tokenized_input],j_call_mask))
    ...
    def _init_raw_signals_encoding_layers(self):
        self.conv1d_layer1 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)
        self.conv1d_layer2 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)
        self.conv1d_layer3 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)
        self.conv1d_layer4 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)

    def _init_masked_v_signals_encoding_layers(self):
        self.conv1d_v_layer_1 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)
        self.conv1d_v_layer_2 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)
        self.conv1d_v_layer_3 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)
        self.conv1d_v_layer_4 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)

    def _init_masked_d_signals_encoding_layers(self):
        self.conv1d_d_layer_1 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)
        self.conv1d_d_layer_2 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)
        self.conv1d_d_layer_3 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)
        self.conv1d_d_layer_4 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)

    def _init_masked_j_signals_encoding_layers(self):
        self.conv1d_j_layer_1 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)
        self.conv1d_j_layer_2 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)
        self.conv1d_j_layer_3 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)
        self.conv1d_j_layer_4 = Conv1D_and_BatchNorm(filters=128, kernel=3, max_pool=2)
    ...
    def _encode_masked_v_signal(self, v_mask_input_embedding):
        v_conv_layer_1 = self.conv1d_v_layer_1(v_mask_input_embedding)
        v_conv_layer_2 = self.conv1d_v_layer_2(v_conv_layer_1)
        v_conv_layer_3 = self.conv1d_v_layer_3(v_conv_layer_2)
        v_feature_map = self.conv1d_v_layer_4(v_conv_layer_3)
        v_feature_map = Flatten()(v_feature_map)
        return v_feature_map

    def _encode_masked_d_signal(self, d_mask_input_embedding):
        d_conv_layer_1 = self.conv1d_d_layer_1(d_mask_input_embedding)
        d_conv_layer_2 = self.conv1d_d_layer_2(d_conv_layer_1)
        d_conv_layer_3 = self.conv1d_d_layer_3(d_conv_layer_2)
        d_feature_map = self.conv1d_d_layer_4(d_conv_layer_3)
        d_feature_map = Flatten()(d_feature_map)
        return d_feature_map

    def _encode_masked_j_signal(self, j_mask_input_embedding):
        j_conv_layer_1 = self.conv1d_j_layer_1(j_mask_input_embedding)
        j_conv_layer_2 = self.conv1d_j_layer_2(j_conv_layer_1)
        j_conv_layer_3 = self.conv1d_j_layer_3(j_conv_layer_2)
        j_feature_map = self.conv1d_j_layer_4(j_conv_layer_3)
        j_feature_map = Flatten()(j_feature_map)
        return j_feature_map

    def call(self, inputs):

        # STEP 1 : Encode tokenized seq to features Via Conv Blocks
        input_embeddings = self.input_embedding_layer(inputs["seq_tokenized"])
        ...
        casted_seq_tokenized = self.reshape_and_cast_input(inputs["seq_tokenized"])
        v_call_masked = self.v_mask_extractor2((casted_seq_tokenized, v_mask))
        d_call_masked = self.d_mask_extractor2((casted_seq_tokenized, d_mask))
        j_call_masked = self.j_mask_extractor2((casted_seq_tokenized, j_mask))

        # STEP 6: Extract new Features
        v_embedding = self.v_mask_embedding_layer(tf.squeeze(v_call_masked, axis=2))
        d_embedding = self.d_mask_embedding_layer(tf.squeeze(d_call_masked, axis=2))
        j_embedding = self.j_mask_embedding_layer(tf.squeeze(j_call_masked, axis=2))

        # Pass The Embeddings Generated Above Thorough 1D Convolutional Feature Extractor Layer
        v_feature_map2 = self._encode_masked_v_signal(v_embedding)
        d_feature_map2 = self._encode_masked_d_signal(d_embedding)
        j_feature_map2 = self._encode_masked_j_signal(j_embedding)
    ...
    def multi_task_loss_v2(self, y_true, y_pred):
        ...
        regression_weight, classification_weight, intersection_weight = (
            0.5,
            0.5,
            0.5,
        ...
        total_loss = (
            ...
            + (intersection_weight * total_intersection_loss)
    ...
    def train_step(self, data):
        ...
        metrics["total_ce_loss"] = self.total_ce_loss_tracker.result()

        return metrics

    def model_summary(self, input_shape):
        x = {
            "seq_tokenized": Input(shape=input_shape),
        }
