...
from VDeepJDataset import VDeepJDataset, VDeepJDatasetSingleBeam
...
class Trainer:
    ...
    def _compile_model(self):
        ...
        if self.optimizers_params is not None:
            self.model.compile(
                optimizer=self.optimizers(**self.optimizers_params),
                loss=None,
                metrics=metrics,
            )
        else:
            self.model.compile(optimizer=self.optimizers(), loss=None, metrics=metrics)
    ...
    def train(self):
        ...
        if self.log_to_file:
            ...
            file_name = (
                ...
                else self.log_file_name
            )
            csv_logger = CSVLogger(
                self.log_file_path + file_name + ".csv", append=True, separator=","
            )
    ...
    def predict(self, eval_dataset_path, raw=True, top_k=1, account_for_padding=True,batch_size=None):
        ...
        if raw:
            ...
            for key in self.train_dataset.reverse_ohe_mapping.keys():
                if top_k == 1:
                    pdf[key] = [
                        self.train_dataset.reverse_ohe_mapping[key][np.argmax(i)]
                        for i in predicted[key]
                    ]

                elif top_k > 1:
                    K = (
                        top_k
                        if top_k < predicted[key].shape[1]
                        else predicted[key].shape[1]
                    )
                    top_k_candidates = []
                    scores = []
                    for row in predicted[key]:
                        ...
                        candidates = [
                            self.train_dataset.reverse_ohe_mapping[key][c]
                            for c in sorted_
                        ]
            ...
            for column in set(predicted.keys()) - set(
                self.train_dataset.reverse_ohe_mapping.keys()
            ):
    ...
    def plot_history(self, write_path=None):
        ...
        ) // num_cols  # Calculate the number of rows based on num_plots
