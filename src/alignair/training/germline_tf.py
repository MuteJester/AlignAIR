...
For each gene, read the true segment's per-position reps (off the backbone reps when
available, else re-encoded through the SHARED encoder as READ) and align them to the true
allele's per-position germline reps (same encoder space) via the seed-and-extend aligner:
the band head places a +-band_width band and the exact banded DP decodes start/end within
it. When ``state_logits`` is supplied, the per-position SHM reliability of the segment is
gathered and passed through so the base-match channel can down-weight likely-mutated positions.
"""
import torch
...
def compute_germline_logits(model, tokens, mask, batch, ref_emb, has_d: bool,
    ...
    Teacher-forced (default): segment is extracted with the TRUE region labels and aligned
    to the coordinate-bearing primary allele's reps. Pass ``region_labels`` (e.g. predicted)
    and/or ``allele_idx`` ({gene_upper: (B,) idx}, e.g. predicted top-1) to measure the true
    end-to-end pipeline instead. Pass ``state_logits`` (B, L, n_states) to enable SHM
    reliability gating in the base-match channel. With ``return_band=True`` also returns the
    per-gene band-offset posteriors (supervised by band_offset_loss)."""
    ...
    for g in genes:
        ...
        # available, else re-encode the segment through the SHARED encoder (READ token type).
        if reps is not None:
            seg_reps, _ = extract_segment(reps, mask, rl, G)                  # (B, S, d), no re-encode
        else:
            seg_reps = model.backbone.forward_positions(
                seg_tok, seg_mask, token_type=SharedNucleotideEncoder.READ)
