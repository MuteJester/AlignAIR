...
def infer_genotype(model, reference, reads, *, locus: str = "IGH", germline_set_ref=None,
    ...
    for gtype in genes:
        ...
        leakage = LeakageModel.fit(W, biases=bias)
        ...
        present = {i for i, m in mass_by_idx.items() if m > present_thr * total}
        ...
        for gene, idxs in groups.items():
            ...
            for i in idxs:
                name = names[i]
                if usage[name]["count"] == 0 and residual[i] < min_support * total * 0.5:
                    continue
                ...
                gene_residual[name] = max(0.0, residual[i]) / total       # fraction of gene-type usage
