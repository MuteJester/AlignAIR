...
class MultiChainAlignAIR(Model):
    ...
    def __init__(self, max_seq_length: int, dataconfigs:MultiDataConfigContainer,
        ...
        Args:
            ...
            dataconfigs (MultiDataConfigContainer): Container holding multiple DataConfig objects
                                                    with allele counts and metadata for different chains.
            ...
            j_allele_latent_size (int, optional): Size of the latent dimension for J-allele
                                                  classification. Defaults to None.
        """
        super(MultiChainAlignAIR, self).__init__()

        # --- Configuration and Parameters ---
        self.max_seq_length = int(max_seq_length)
        self.dataconfigs = dataconfigs
        self.has_d_gene = self.dataconfigs.has_at_least_one_d()

        self.initializer = keras.initializers.GlorotUniform()

        # Allele counts and latent sizes
        self.v_allele_count = self.dataconfigs.number_of_v_alleles
        self.j_allele_count = self.dataconfigs.number_of_j_alleles
        self.v_allele_latent_size = v_allele_latent_size
        self.j_allele_latent_size = j_allele_latent_size

        self.number_of_types = len(self.dataconfigs.chain_types())

        if self.has_d_gene:
            self.d_allele_count = self.dataconfigs.number_of_d_alleles
    ...
    def setup_log_variances(self):
        ...
        self.log_var_chain_type = RegularizedConstrainedLogVar('log_var_chain_type')
    ...
    def setup_performance_metrics(self):
        ...
        self.scaled_chain_type_loss_tracker = keras.metrics.Mean(name="scaled_chain_type_loss")
        ...
        # Epoch-aggregated AUC metrics for allele heads (treat multi-class as multi-label)
    ...
    def _init_segmentation_heads(self):
        ...
        # Position-logit heads (Dense(L) per boundary). Softmax is applied in call().
    ...
    def _init_analysis_heads(self):
        ...
        self.chain_type_head = Dense(self.number_of_types, activation='softmax', name="chain_type", kernel_initializer=self.initializer)
    ...
    def call(self, inputs, training=False):
        ...
        v_start = tf.reduce_sum(v_start_probs * positions, axis=-1, keepdims=True)
        v_end = tf.reduce_sum(v_end_probs * positions, axis=-1, keepdims=True)
        j_start = tf.reduce_sum(j_start_probs * positions, axis=-1, keepdims=True)
        j_end = tf.reduce_sum(j_end_probs * positions, axis=-1, keepdims=True)
        ...
        chain_type_mid = self.chain_type_dropout(chain_type_mid, training=training)
        ...
        if self.has_d_gene:
            ...
            d_start = tf.reduce_sum(d_start_probs * positions, axis=-1, keepdims=True)
            d_end = tf.reduce_sum(d_end_probs * positions, axis=-1, keepdims=True)
        ...
        # Expose logits for CE training; keep expectations for backward-compat/metrics
        output = {
        ...
        if self.has_d_gene:
            output.update({
                ...
                'd_start': d_start, 'd_end': d_end, 'd_allele': d_allele
            })
    ...
    def hierarchical_loss(self, y_true, y_pred):
        ...
        v_start_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=v_start_t, logits=y_pred['v_start_logits']))
        v_end_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=v_end_t, logits=y_pred['v_end_logits']))

        # J boundaries
        j_start_t = soft_targets(y_true['j_start'], L)
        j_end_t = soft_targets(y_true['j_end'], L)
        j_start_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=j_start_t, logits=y_pred['j_start_logits']))
        j_end_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=j_end_t, logits=y_pred['j_end_logits']))
        ...
        if self.has_d_gene:
            ...
            d_start_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=d_start_t, logits=y_pred['d_start_logits']))
            d_end_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=d_end_t, logits=y_pred['d_end_logits']))
        ...
        weighted_chain_type_loss = chain_type_loss * self.log_var_chain_type(chain_type_loss)
    ...
    def log_metrics(self, total_loss, scaled_classification_loss, scaled_indel_count_loss,
                    scaled_mutation_rate_loss, segmentation_loss, scaled_productive_loss,weighted_chain_type_loss):
        ...
        self.scaled_chain_type_loss_tracker.update_state(weighted_chain_type_loss)
    ...
    def update_custom_trackers(self, y_true, y_pred):
        ...
        # Allele AUC metrics (multi-label)
        try:
            ...
            # be robust to shape/key mismatches in older datasets
            pass
    ...
    def get_metrics_log(self):
        ...
        metrics.update({
            ...
            'chain_type_loss': self.scaled_chain_type_loss_tracker.result(),
    ...
    def metrics(self):
        ...
        *** THIS IS THE CRITICAL FIX ***
        ...
        metric_list = [
            ...
            self.scaled_chain_type_loss_tracker,
    ...
    def save_pretrained(self, bundle_dir: Union[str, os.PathLike], training_meta: Optional[TrainingMeta] = None,
        ...
        # Write bundle metadata first (config, dataconfig, training_meta); fingerprint will be overwritten after export
    ...
    def from_pretrained(cls, bundle_dir: Union[str, os.PathLike]):
        """Load a multi-chain model from a SavedModel-first bundle. H5 is no longer supported."""
        bundle_path = Path(bundle_dir)
        ...
        if sm_dir.exists():
            ...
            wrapper = SavedModelInferenceWrapper(saved_model_dir=sm_dir, bundle_dir=bundle_path, config=cfg.__dict__ if hasattr(cfg, '__dict__') else None)
            wrapper.dataconfigs = dataconfigs_obj
            logging.getLogger(__name__).info("Loaded multi-chain SavedModel from %s", sm_dir)
            return wrapper
    ...
    def export_saved_model(self, export_dir: Union[str, os.PathLike], include_logits: bool = False):
        ...
        class _InferenceModule(tf.Module):  # pragma: no cover - export
            def __init__(self, model, max_len: int, include_logits_flag: bool):
                ...
                self.serving_default = tf.function(
                    _serving,
                    input_signature=[tf.TensorSpec([None, self.L], tf.int32, 'tokenized_sequence')]
                )
