...
class SequenceCorruptor:
    def __init__(self, nucleotide_add_coef=100, nucleotide_remove_coef=100,nucleotide_add_remove_coef=50,max_length=512,
                 random_sequence_add_proba=1,single_base_stream_proba=0,duplicate_leading_proba=0,
                 random_allele_proba=0,corrupt_proba=1,
                 nucleotide_add_distribution=None,
                 nucleotide_remove_distribution=None,
...
class PositionContainer:
    ...
    def insert(self, position):
        # Check if the insertion is within the range of the container
        if position < self.v_start or position > self.j_end:
            raise ValueError("Insertion position is out of range")

        # If the insertion is within the 'v' segment or after it
        if position > self.v_start:
            self.v_end += 1
            self.d_start += 1
            self.d_end += 1
            self.j_start += 1
        ...
        # If the insertion is within the 'd' segment or after it
        if position >= self.d_start:
            self.d_end += 1
            self.j_start += 1
            self.j_end += 1

        # If the insertion is within the 'j' segment or after it
        if position >= self.j_start:
            self.j_end += 1
    def delete(self, position):
        ...
        # If the deletion is within the 'v' segment or after it
        if position > self.v_start:
            self.v_end -= 1
            self.d_start -= 1
            self.d_end -= 1
        ...
        # If the deletion is within the 'd' segment or after it
        if position >= self.d_start:
            self.d_end -= 1
            self.j_start -= 1
            self.j_end -= 1

        # If the deletion is within the 'j' segment or after it
        if position >= self.j_start:
            self.j_end -= 1

    def sample_position(self):
        # Choose a segment uniformly
        segment = random.choice(['v', 'd', 'j'])

        if segment == 'v':
            return 'v',random.randint(self.v_start, self.v_end)
        elif segment == 'd':
            return 'd',random.randint(self.d_start, self.d_end)
        else:  # segment == 'j'
            return 'j',random.randint(self.j_start, self.j_end)

    def insert_nucleotide(self,position, sequence):
        # Check if the position is valid
        if position < 0 or position > len(sequence):
            raise ValueError("Position is out of range")

        nucleotide = random.choice(['A','T','C','G'])
        # Insert the nucleotide at the specified position
        self.insert(position)
        return sequence[:position] + nucleotide + sequence[position:]
    def delete_nucleotide(self,position, sequence):
        # Check if the position is valid
        if position < 0 or position >= len(sequence):
            raise ValueError("Position is out of range")
        self.delete(position)
        # Remove the nucleotide at the specified position
...
class SequenceCorruptorV2:
    ...
    def __init__(self, nucleotide_add_coef=100, nucleotide_remove_coef=100,nucleotide_add_remove_coef=50,max_length=512,
        ...
        self.nucleotide_remove_distribution = st.beta(2, 3) if nucleotide_remove_distribution is None else nucleotide_remove_distribution
        self.nucleotide_add_remove_distribution = st.beta(1, 3)
        self.deletion_amount_distribution = st.beta(1, 3) if deletion_amount_distribution is None else deletion_amount_distribution
        self.insertion_amount_distribution = st.beta(1, 3) if insertion_amount_distribution is None else insertion_amount_distribution
        self.corrupt_proba = corrupt_proba
    ...
    def _deletion_amount_distribution(self, size):
        sample = (1+(self.deletion_amount_distribution.rvs(1)*self.deletion_coef).astype(int)).item()
        if len(sample) == 1:
            return sample.item()
        else:
            return sample
    def _insertion_amount_distribution(self, size):
        sample = (1+(self.insertion_amount_distribution.rvs(1)*self.insertion_coef).astype(int)).item()
        if len(sample) == 1:
            return sample.item()
        else:
            return sample
            ...
            if corrupt_beginning and row.to_corrupt:
                ...
                if np.random.binomial(1, self.insertion_proba) == 1:
                    ...
                    for i in range(amount_to_insert):
                        ...
                        random_segment, random_position = position_container.sample_position()
                        # update sequence
                        seq = position_container.insert_nucleotide(random_position, seq)
                        # save position for mask adjustment
                        insertion_positions[random_segment].append(random_position)
                ...
                if np.random.binomial(1, self.deletions_proba) == 1:
                    ...
                    for i in range(amount_to_delete):
                        ...
                        random_segment, random_position = position_container.sample_position()
                        # update sequence
                        seq = position_container.delete_nucleotide(random_position, seq)
                        # raise flag
