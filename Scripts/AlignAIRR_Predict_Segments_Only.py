...
def main():
    ...
    lcd = HeavyChainDataset(data_path=args.csv_file,
    ...
    try:
        data = pd.read_csv(args.csv_file, usecols=['sequence','v_call','d_call','j_call'], sep=sep)
    except ValueError:
        data = pd.read_csv(args.csv_file, usecols=['sequence','v_allele','d_allele','j_allele'], sep=sep)
        data = data.rename(columns={'v_allele':'v_call','d_allele':'d_call','j_allele':'j_call'})
    ...
    for i in tqdm(dataset):
        pred = trainer.model.predict(i, verbose=False, batch_size=1024)
    ...
    for seg in seg_map:
        with open(args.save_path + file_name+f'_{seg}_'+  'alignairr_segments.pkl', 'wb') as h:
                        pickle.dump(seg_map[seg], h)
