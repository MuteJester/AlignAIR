...
class VDeepJAllignExperimentalSingleBeamConvSegmentationResidual_DC_MR(tf.keras.Model):
    ...
    def _init_segmentation_layers(self):
        ...
        self.mutation_rate_head = Dense(
            1, activation='relu', name="mutation_rate", kernel_initializer=self.initializer,kernel_constraint=MinMaxValueConstraint(0, 1)
        )
...
class VDeepJAllignExperimentalSingleBeamConvSegmentationResidual_DC_MR_ONLY_ALLELES(tf.keras.Model):
    """
    this model replaces the transformer blocks back to Conv Blocks
    and replace mask logic with start and end regression to mask prediction as in actual image segmentation
    tasks
    ...
    def __init__(
        ...
        super(VDeepJAllignExperimentalSingleBeamConvSegmentationResidual_DC_MR_ONLY_ALLELES, self).__init__()
...
class VDeepJAlignExperimentalSingleBeamConvSegmentationResidualV3(tf.keras.Model):
    ...
    def __init__(
        ...
        super(VDeepJAlignExperimentalSingleBeamConvSegmentationResidualV3, self).__init__()
    ...
    def _init_v_classification_layers(self):
        ...
        self.v_deletion_mid = Dense(
            self.max_seq_length//2,
            activation='swish',
            name="v_deletion_middle", kernel_initializer=self.initializer,
        )
        self.v_deletion = Dense(
            1, activation="sigmoid", name="v_deletion"
        )
    ...
    def _init_j_classification_layers(self):
        ...
        self.j_deletion_mid = Dense(
            self.max_seq_length // 2,
            activation='swish',
            name="j_deletion_middle", kernel_initializer=self.initializer,
        )
        self.j_deletion = Dense(
            1, activation="sigmoid", name="j_deletion"
        )
    ...
    def _init_segmentation_layers(self):
        ...
        self.d_segment_mid = Dense(
            64, activation=act, kernel_constraint=unit_norm(), kernel_initializer=self.initializer,
        )  # (concatenated_path)
        self.d_segment_out = Dense(self.max_seq_length, activation="sigmoid", name="d_segment",
                                   kernel_initializer=self.initializer)  # (d_start_mid)
        ...
        self.mutation_rate_mid = Dense(
            64, activation=act, name="mutation_rate_mid", kernel_initializer=self.initializer
        )
        self.mutation_rate_dropout = Dropout(0.2)
        self.mutation_rate_head = Dense(
            1, activation="sigmoid", name="mutation_rate", kernel_initializer=self.initializer
        )
    ...
    def _predict_vdj_set(self, v_feature_map, d_feature_map, j_feature_map):
        ...
        v_deletions_middle = self.v_allele_mid(v_feature_map)
        v_deletions = self.v_deletion(v_deletions_middle)
        ...
        j_deletions_middle = self.j_allele_mid(j_feature_map)
        j_deletions = self.j_deletion(j_deletions_middle)

        return v_allele, d_allele, j_allele, v_deletions, j_deletions
    ...
    def call(self, inputs):
        ...
        # STEP 8: Predict The V,D and J genes
        v_allele, d_allele, j_allele, v_deletions, j_deletions \
            = self._predict_vdj_set(v_feature_map, d_feature_map, j_feature_map)
        return {
            "v_segment": v_segment,
            "d_segment": d_segment,
