...
class Sequence:
    ...
    def get_nuc_seq(self, no_trim_list, trim_dicts, no_np_list, NP_lengths, NP_transitions, NP_first_bases, gapped=False):
        ...
        if gapped == True:
            v_trimmed_seq,v_trim_5,v_trim_3 = get_gapped_trimmed_seq(v_allele, no_trim_list, trim_dicts)
            d_trimmed_seq,d_trim_5,d_trim_3 = get_gapped_trimmed_seq(d_allele, no_trim_list, trim_dicts)
            j_trimmed_seq,j_trim_5,j_trim_3 = get_gapped_trimmed_seq(j_allele, no_trim_list, trim_dicts)
        ...
        if gapped == False:
            v_trimmed_seq, v_trim_5, v_trim_3 = get_trimmed_seq(v_allele, no_trim_list, trim_dicts)
            d_trimmed_seq, d_trim_5, d_trim_3 = get_trimmed_seq(d_allele, no_trim_list, trim_dicts)
            j_trimmed_seq, j_trim_5, j_trim_3 = get_trimmed_seq(j_allele, no_trim_list, trim_dicts)
            nuc_seq = (
                v_trimmed_seq
                + self.NP1_region
                + d_trimmed_seq
...
def generate_sequence_spesific(v,d,j, data_dict, mutate=False, flat_usage=False, no_trim_list=(False, False, False, False, False), no_np_list=(False, False, False), shm_flat=False, shm_random=False, mutation_rate=None, mutation_number=None):
    """Wrapper to bring together entire sequence generation process.
...
def alignairr_generate_sequence(locus, data_dict, mutate=False, flat_usage=False, no_trim_list=(False, False, False, False, False), no_np_list=(False, False, False), shm_flat=False, shm_random=False, mutation_rate=None, mutation_number=None):
    gene_use_dict = data_dict["gene_use_dict"]
    family_use_dict = data_dict["family_use_dict"]
    trim_dicts = data_dict["trim_dicts"]
    NP_lengths = data_dict["NP_lengths"]
    ...
    if mutate == True:
        ...
        sequence.mutations = ""
        sequence.mutated_seq, sequence.mutations, mutation_rate = overarch_mutation(
            sequence.gapped_seq, sequence.v_allele.family, sequence.junction_length, kmer_dicts, shm_flat, shm_random,
            mut_rate_per_seq, mut_rate=mutation_rate, mut_num=mutation_number)
        sequence.junction = sequence.mutated_seq[sequence.v_allele.anchor:
                                                 sequence.v_allele.anchor + sequence.junction_length].upper()
...
if __name__ == "__main__":
    ...
    # flat_usage = 'gene'
    # shm_flat = True
    # path_to_data = 'C:/Users/Tomas/anaconda3/Lib/site-packages/airrship/data/'
    # data_dict = load_data(path_to_data, mutate)
    # locus = global_genotype()
    # samples = []
    # for _ in tqdm(range(3500)):
    #     S = generate_sequence(locus, data_dict, mutate=True,
    #                                   mutation_rate=0.5,
    #                                   shm_flat=False, flat_usage='allele')
    #     samples.append(S.mut_freq)