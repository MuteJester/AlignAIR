import unittest
from SequenceSimulation.sequence import LightChainSequence, HeavyChainSequence
import pickle
from importlib import resources

with resources.path('SequenceSimulation.data', 'LightChain_KAPPA_DataConfigV2.pkl') as data_path:
    with open(data_path, 'rb') as h:
        lightchain_kappa_config = pickle.load(h)

with resources.path('SequenceSimulation.data', 'LightChain_LAMBDA_DataConfigV2.pkl') as data_path:
    with open(data_path, 'rb') as h:
        lightchain_lambda_config = pickle.load(h)

with resources.path('SequenceSimulation.data', 'HeavyChain_DataConfig_OGRDB_V2.pkl') as data_path:
    with open(data_path, 'rb') as h:
...
class TestSequenceSimulation(unittest.TestCase):
    ...
    def test_lighchain_sequence_simulator(self):
        from SequenceSimulation.simulation import LightChainKappaLambdaSequenceAugmentor, SequenceAugmentorArguments
