...
def test_singlechain_logits_present_for_loss():
    ...
    out = AlignAIR(cfg).eval()({"tokenized_sequence": torch.randint(0, 6, (2, 256))})
...
def test_clamp_params_bounds_analysis_heads():
    ...
    m = AlignAIR(cfg)
    m.meta_heads["mutation_rate"].head.weight.data.fill_(5.0)
    m.meta_heads["indel_count"].head.weight.data.fill_(100.0)
    m.clamp_params()
    assert m.meta_heads["mutation_rate"].head.weight.max().item() <= 1.0
    assert m.meta_heads["indel_count"].head.weight.max().item() <= 50.0
